  ...
  users: {
    login: async (req, res) => {
      ...
      try {
        ...
        if (user) {
          res.status(200).send({
            code: 0,
            token: tokenSign({ id: user.id, email })
          });
        } else {
          res.status(409).send({
            code: 1,
            message: "email/password 일치하지 않음"
          });
        }
      } catch {
        res.status(500).send({
    ...
    register: async (req, res) => {
      ...
      try {
        ...
        if (!user) {
          ...
          res.status(200).send({
            code: 0,
            token: tokenSign({ id: result.id, email })
          });
        } else {
          res.status(409).send({
            code: 1,
            message: "email 이미 존재함"
          });
        }
      } catch {
        res.status(500).send({
    ...
    editPassword: async (req, res) => {
      ...
      try {
        ...
        if (user) {
          await Users.update(
            { password: cryptoPassword(password2) },
            { where: { email } }
          );
          res.status(200).send({
            code: 0,
    ...
    editUsername: async (req, res) => {
      ...
      let { email } = req.user;
      try {
        await Users.update({ username }, { where: { email } });
        res.status(200).send({
          code: 0,
          message: "유저네임 수정완료"
        });
    ...
    checkToken: async (req, res, next) => {
      ...
      if (token) {
        if (tokenVerify(token)) {
          try {
            req.user = await jwt.verify(token, secret.secret);
            next();
          } catch {
            res.status(401).send({
              code: 1,
    ...
    getContainers: async (req, res) => {
      ...
      try {
        let containers = await Containers.findAll({
          ...
          include: [{ model: Cards, attributes: ["id", "cardName"] }],
          where: { board_id }
        });
        res.status(200).send({
          code: 0,
